...
def vicrop_qa(model_name, method_name, image_path, question, model, processor, short_question):
    ...
    Returns:
        tuple: (original_answer, cropped_answer, bounding_box, crop_metadata)
    ...
    if model_name == "llava":
        ...
        if method_name == 'grad_att':
            ...
            bbox, crop_meta = bbox_from_att_image_adaptive(att_map, image.size, bbox_size, return_metadata=True)
            ...
            bbox, crop_meta = bbox_from_att_image_adaptive(att_maps, image.size, bbox_size, return_metadata=True)
        ...
        #------------------------------------------------------------------------------------------------------------------------------------
            ...
            bbox, crop_meta = bbox_from_att_image_adaptive(grad, image.size, bbox_size, return_metadata=True)

        elif method_name == 'pure_grad_high':
            grads = high_res(pure_gradient_llava, image, short_prompt, general_prompt, model, processor)
            bbox, crop_meta = bbox_from_att_image_adaptive(grads, image.size, bbox_size, return_metadata=True)
        ...
        return ori_generation, multi_generation, bbox, crop_meta
        ...
        ori_num_img_tokens = int((ori_inputs.input_ids[0] == 151655).sum().item())
        ...
        return ori_generation, multi_generation, bbox, crop_meta, num_img_tokens
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default="rel_att", choices=["rel_att", "pure_grad", "grad_att", "rel_att_high", "pure_grad_high", "grad_att_high"])
